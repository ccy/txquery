...
{$IFDEF LEVEL6}
  , Variants
{$ENDIF}
  , xqTypes; {added by fduenas: conatins base types and classes that can be used in any unit}
...
Begin
  Result := secDir;
  ReallocMem( Result, newSlots * {$IFNDEF XQ_USE_SIZEOF_CONSTANTS}SizeOf(Pointer){$ELSE}XQ_SizeOf_Pointer{$ENDIF} );
  FillChar( Result^[slotsInDir], ( newSlots - slotsInDir ) * {$IFNDEF XQ_USE_SIZEOF_CONSTANTS}SizeOf(Pointer){$ELSE}XQ_SizeOf_Pointer{$ENDIF}, #0 );
  slotsInDir := newSlots;
End;
...
Var
  ...
  Size: Word;
Begin
  Size := SectionSize * {$IFNDEF XQ_USE_SIZEOF_CONSTANTS}SizeOf(Pointer){$ELSE}XQ_SizeOf_Pointer{$ENDIF};
  GetMem( secP, size );
...
Begin
  ...
  i := 0;
  size := FSectionSize * {$IFNDEF XQ_USE_SIZEOF_CONSTANTS}SizeOf(Pointer){$ELSE}XQ_SizeOf_Pointer{$ENDIF};
  ...
  If secDir <> Nil Then
    FreeMem( secDir, slotsInDir * {$IFNDEF XQ_USE_SIZEOF_CONSTANTS}SizeOf(Pointer){$ELSE}XQ_SizeOf_Pointer{$ENDIF} );
End;

Function TSparsePointerArray.GetAt( Index: Integer ): Pointer;
Var
  byteP: PChar; { patched by ccy }
  secIndex: Cardinal;
Begin
